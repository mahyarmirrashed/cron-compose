...
describe("DayOfWeekSlot", () => {
  ...
  it("should add day of week enum range correctly and return *", () => {
    slot.addRange("sun", "sat");
    expect(slot.toString()).toBe("*");
  });
  ...
  it("should remove number range correctly", () => {
    slot.addRange(0, 6);
    slot.removeRange(2, 4);
    expect(slot.toString()).toBe("0-1,5-6");
  });
